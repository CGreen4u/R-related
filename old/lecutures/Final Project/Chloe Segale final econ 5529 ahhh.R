...
real<lower=0,upper=100> sigma_rho;
...
mu_rho ~ normal(0, 1);
rho ~ normal(0.1*mu_rho, sigma_rho);
g ~ normal(g_hat, sigma_g); //specified priors

}"

dat <- list(N=nrow(data), J=J, firm=data$id, g=log(data$equation), k=(data$growth))


stan.out <- stan(model_code = model, data = data, iter = 2000, chains = 3,warmup = 500)
...
dat<- list(N=nrow(firmdata),J=J,y=data$growth)
