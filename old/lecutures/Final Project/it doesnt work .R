...
int<lower=1> J; //number of firms - can lower bound be 54
vector[N] g; //growth per observations 
 //vector[N] k; //use k as transformed t-1 variable??
int<lower=1,upper=J> firm[N]; //vector of observations in each firm
}
parameters {
...
int<lower=1,upper=J> firm[N] // /vector of observationss per each firm
vector[N] y;
}
parameters {
real alpha;
